...
- ✅ **v91 — Login Google volta para o jogo + conta GitHub renomeada (arcana-devtools)**:
  ...
  - ⚠️ **Ação do usuário (Supabase)**: o login com Google também exige autorizar o domínio novo no painel do Supabase (projeto `fntwsizxffulzgmspwto`) → Authentication → URL Configuration: Site URL = `https://arcana-devtools.github.io/cronicas-de-arcana/` e Redirect URLs adicionar `https://arcana-devtools.github.io/**` (mais o `http://localhost:**` para testes). O provider Google continua habilitado (callback do Supabase inalterado). Login como convidado e conta local (nome/senha) funcionam sem isso. `node --check` OK. sw.js → `arcana-v91`.
...
- ✅ **v83 — Monstros mais bonitos e com volume (estilo Curse of Aros)**:
  ...
  - **Brilho no olhar (catchlight)** (`drawEnemyCatchlight`, nova): pontinho branco no alto dos olhos, no lado para que o bicho está virado (usa `e.dirX/dirY`), com piscada rara. Dá "vida" aos bichos. Chamado **depois** do `ctx.restore()` do `drawEnemy` (fora do espelhamento `ctx.scale(flip)`) pra não sair no lado errado quando vira pra esquerda. Pula fantasmas/gosmas/elementais/golens (sem rosto) e só na qualidade alta.
  - Tudo é puramente visual (luz/sombra/brilho), sem tocar em IA, combate, colisão ou save. Marcadores: `volume "assado" no sprite`, `reflexo de cor no chão`, `pontinho de brilho no olhar`, `drawEnemyCatchlight`. Save 100% compatível. sw.js → `arcana-v83`.

- ✅ **v82 — Terreno mais rico e realista (estilo Curse of Aros)**:
  - Nova **camada de detalhes do chão por bioma** dentro do loop do `drawGround` (tiles de 48px), só em `visualLevel()>0` (médio/alto; no modo econômico mobile some para não pesar). Por bioma: **floresta** (início) ganha tufos de grama em pinceladas, **flores coloridas** (rosa/amarelo/branco/laranja) e **pedrinhas** com brilho; **deserto/areia** ganha pedrinhas arredondadas + torrões; **neve** ganha cristais de gelo e brilho azul; **bosque de cristais** ganha lascas de cristal ciano/roxo que brilham; **pântano** ganha tufos escuros + bolhas; **tempestade** ganha grama azulada; zonas mágicas (caçada/santuário) ganham brilhinho dourado. Também há **manchas suaves de relevo** (claras/escuras) em ~1/7 dos tiles, dando profundidade ao chão. Densidade maior no high (`dens=3`) e esparsa no medium (`dens=5`). Detalhes 100% determinísticos via `hash(x,y)` (não piscam ao andar).
  - **Árvores** (`drawTree`): no high, brilho suave no topo da copa (luz de cima → profundidade); na floresta, **tufos de grama na base do tronco**.
  - Tudo é só render visual (mesmas cores/base), sem tocar em save nem em colisão. Marcadores: `detalhes realistas de terreno por bioma`, `luz suave no topo da copa`. Save 100% compatível. sw.js → `arcana-v82`.

- ✅ **v81 — Escudo passa a ACOMPANHAR a mira no celular (não fica mais travado)**:
  - Bug (Ronald): ao defender, o escudo ficava preso numa direção e não bloqueava ataques que vinham de outro lado. Causa: tanto o bloqueio (`shieldFacesPoint` via `combatState.guardAngle`) quanto o desenho (`drawPlayerShield`) usavam `playerFacingVector()`, que no mobile cai em `player.dirX/dirY` (direção do CORPO/movimento) — quando o jogador fica parado ou mira com o analógico de ataque (twin-stick), o corpo não vira, então o escudo não acompanha a mira nem os inimigos.
  - Fix: novo helper `guardFacingVector()` — prioriza (1) o analógico de ataque arrastado (`attackJoystick`), (2) o cursor (desktop), (3) o alvo travado do auto-aim mobile (`mobileAimTarget()`), (4) por fim a direção do corpo. Usado em `beginShieldGuard`, na atualização do `guardAngle` (dentro do `update`, no bloco `if(combatState.guardActive)`) e no desenho do escudo visual em `drawPlayer` (as duas chamadas de `drawPlayerShield` agora usam `gFaceX/gFaceY`). O corpo do herói continua virando pelo movimento; só o escudo é que aponta para a mira/ameaça.
  - Marcadores p/ curl: `guardFacingVector`, `o escudo acompanha a MIRA`. Save 100% compatível. sw.js → `arcana-v81`.
